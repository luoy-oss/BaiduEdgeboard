...
using namespace cv;
uint8_t img_data[ROWSIMAGE/IMAGESCALE][COLSIMAGE/IMAGESCALE];
image_t img_raw((uint8_t*)img_data, COLSIMAGE/IMAGESCALE, ROWSIMAGE/IMAGESCALE);
void frameTOimg_raw(const Mat& frame) {
    for (int c = 0; c < COLSIMAGE/IMAGESCALE; c++) {
        for (int r = 0; r < ROWSIMAGE/IMAGESCALE; r++) {
            AT_IMAGE(&img_raw, (int)c, (int)r) = (int)frame.at<uchar>(r * IMAGESCALE, c * IMAGESCALE);
        }
    }
}
void process_image() {
    // 原图找左右边线
    int x1 = COLSIMAGE/IMAGESCALE / 2 - begin_x, y1 = begin_y;
    ...
    int x2 = COLSIMAGE/IMAGESCALE / 2 + begin_x, y2 = begin_y;
    ipts1_num = sizeof(ipts1) / sizeof(ipts1[0]);
    for (; x2 < COLSIMAGE/IMAGESCALE - 1; x2++) if (AT_IMAGE(&img_raw, x2 + 1, y2) < thres) break;
    ...
    for (int i = 0; i < ipts0_num; i++) {
        rpts0[i][0] = mapx[ipts0[i][1] * IMAGESCALE][ipts0[i][0] * IMAGESCALE] / IMAGESCALE;
        rpts0[i][1] = mapy[ipts0[i][1] * IMAGESCALE][ipts0[i][0] * IMAGESCALE] / IMAGESCALE;
    }
    rpts0_num = ipts0_num;
    for (int i = 0; i < ipts1_num; i++) {
        rpts1[i][0] = mapx[ipts1[i][1] * IMAGESCALE][ipts1[i][0] * IMAGESCALE] / IMAGESCALE;
        rpts1[i][1] = mapy[ipts1[i][1] * IMAGESCALE][ipts1[i][0] * IMAGESCALE] / IMAGESCALE;
    }
    rpts1_num = ipts1_num;
    ...
    vector<int> v_slope;
    int step = 10 / IMAGESCALE; // size/10;

    if (rpts1s_num < ROWSIMAGE/IMAGESCALE / 4) {
        stdevRight = 1000;
    }
    if (rpts1s_num > 100 / IMAGESCALE) {
        for (int i = step; i < 100 / IMAGESCALE; i += step) {
    ...
    if (rpts0s_num < ROWSIMAGE/IMAGESCALE / 4) {
        stdevLeft = 1000;
    }

    v_slope.clear();
    if (rpts0s_num > 100 / IMAGESCALE) {
        for (int i = step; i < 100 / IMAGESCALE; i += step) {
...
void show_line() {
    ...
    resize(nitoushi, nitoushi, Size(COLSIMAGE, ROWSIMAGE));
#ifdef CAR_SAVEIMG
    static int i = 1;
